import os


dataset = ['manhattan']#['csail', 'manhattan', 'intel'] #, 'mit']
inliers_quantity = [1952]#[127, 1952, 256] #, 20]
poses_quantities = [3500]
inliers_percentages = [0.5, 0.6, 0.7, 0.8, 0.9]
sample_size = 10
method = 'vertigo'

outliers_quantity = []
for i in range(0, len(dataset)):
    outliers_quantity.append([])
    for inlier_percentage in inliers_percentages:
        inlier_n = inliers_quantity[i]
        outliers_quantity[-1].append(round(inlier_n / inlier_percentage - inlier_n))
#print(outliers_quantity)

os.chdir('/home/amber/stew/test_backend/'+dataset[0])

for i in range(0,len(dataset)):
    for j in range(0, len(inliers_percentages)):

        dataset_name = dataset[i]
        configuration_name = 'random' + str(outliers_quantity[i][j])
        file_list_string = ''
        for k in range(1, sample_size+1):
            
            os.chdir(method + '_' + dataset_name + '_' + configuration_name + '/' + dataset_name + '_seed_' + str(k) + '/')
            command = './analyze_switch_variable.sh ' + str(poses_quantities[i]) + ' ' + str(inliers_quantity[i]) + ' ' + str(outliers_quantity[i][j])
            print (command)
            os.system(command)
            os.system('mv switch_variable_analysis.txt switch_variable_analysis_' + str(k) + '.txt')
            os.system('cp switch_variable_analysis_' + str(k) + '.txt ../')
            file_list_string += ' switch_variable_analysis_' + str(k) + '.txt'
            os.chdir('../../')

        os.chdir(method + '_' + dataset_name + '_' + configuration_name)
        os.system('cat' + file_list_string + ' > switch_variable_analysis_' + configuration_name)
        os.system('cp switch_variable_analysis_' + configuration_name + ' ../')
        os.chdir('../')
        os.system('grep \'inliers acceptted\' switch_variable_analysis_' + configuration_name + ' > ' + method + '_inlier_' + configuration_name)
        os.system('grep \'outliers acceptted\' switch_variable_analysis_' + configuration_name + ' > ' + method + '_outlier_' + configuration_name)