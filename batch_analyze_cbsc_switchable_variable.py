...
for i in range(0,len(dataset)):
    for j in range(0, len(inliers_percentages)):
        ...
        for k in range(1, sample_size+1):
            ...
            command = './analyze_cbsc_switch_variable.sh ' + str(poses_quantities[i]) + ' ' + str(inliers_quantity[i]) + ' ' + str(outliers_quantity[i][j])
            print (command)
            os.system(command)
            os.system('mv switch_variable_analysis.txt cbsc_switch_variable_analysis_' + str(k) + '.txt')
            os.system('cp cbsc_switch_variable_analysis_' + str(k) + '.txt ../')
            file_list_string += ' cbsc_switch_variable_analysis_' + str(k) + '.txt'
            os.chdir('../../')
